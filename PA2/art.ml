...
let rec build (rand,depth) = 
  match depth with
  (* If 0, need to match to either x or y. Figure a way to choose *)
  0 -> if (rand(1,6) mod 2) = 1 then buildX() else buildY()
  (* now compare the result of rand to any of the expressions *)
  | _ -> let rand1 = ((rand(1,6)*5) mod 7) in
         ...
         | 2 -> buildThresh((build(rand,depth')), (build(rand,depth')), 
 ...
 * Build an expression tree. The second argument is the depth,
 * the first is a random function. A call to rand(2,5) will give you a 
 * random number in the range [2,5) (2 inclusive, and 5 exclusive).
 *)
let rec build2 (rand,depth) = 
  ...
  0 -> if (rand(1,6) mod 2) = 1 then buildX() else buildY()
  (* now compare the result of rand to any of the expressions *)
  | _ -> let rand1 = ((rand(1,8))) in
         let depth' = depth-1 in
         match rand1 with
         | 1 -> buildSine(build (rand,depth'))
         | 2 -> buildCosine(build (rand,depth'))
         | 3 -> buildAverage((build (rand, depth')),(build(rand,depth')))
         | 4 -> buildTimes((build(rand,depth')),(build(rand,depth')))
         | 5 -> buildThresh((build(rand,depth')),(build(rand,depth')),
                (build(rand,depth')),(build(rand,depth')))
         | 6 -> buildPedroExpr1((build(rand,depth')),(build(rand,depth')),
                (build(rand,depth')))
         | 7 -> buildPedroExpr2((build(rand,depth')))
;;
